...
FRIEND_CODE_REGEX = re.compile(r"\b([A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{5})\b", re.IGNORECASE)
...
class FriendCodes(commands.Cog):
    ...
    async def get_friend_code(self, user: discord.User):
        async with aiosqlite.connect(DB_FILE) as db:
            async with db.execute(
                "SELECT friend_code FROM friendcodes WHERE user_id = ?",
                (user.id,)
            ) as cursor:
    ...
    async def on_message(self, message: discord.Message):
        ...
        match = FRIEND_CODE_REGEX.search(message.content)
        if not match:
            return
